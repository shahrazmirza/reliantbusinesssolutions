          ...
          >
            {items.map(item => (
              <li key={item.id}>
                ...
                variants={{ 
                  initial: { y: 50, opacity:0 },
                  animate: { y: 0, opacity: 1, transition: { duration: 0.5, delay: 1.5 }}
                }}
                initial = 'initial'
                animate={fadeControls}
                className='p-8 border-solid border'>
                  <Link href={item.href} className='md:text-2xl/snug font-medium text-xl/snug'>{item.title}</Link>
                  <p className='text-l/snug font-normal text-gray-500 py-5'>{item.content}</p>
                  <Link href={item.href} className="px-5 text-sm font-medium leading-none border-sky-700 border-solid border rounded text-white hover:text-sky-700 h-10 hover:bg-white bg-sky-700 flex items-center justify-center w-fit">Read More</Link>
                </motion.div>
              </li>
            ))}
          </ul>
