  ...
  return (
    <div className='md:pt-4 md:pb-16 pb-10 bg-sky-100'>
      <Container>
          ...
          >
              {items.map(item => (
                  <li key={item.id}>
                      ...
                      <div className='text-sky-700 pb-5' dangerouslySetInnerHTML={{__html: item.icon.replace('<svg', `<svg width="40" height="40"`)}}/>
                        <p className='text-2xl/snug font-medium'>{item.title}
                        </p>
                        <p className='text-l/snug font-normal text-gray-500 pt-5 md:pb-5'>{item.content}</p>
                      </div>
                  </li>
              ))}
          </motion.ul>
        ...
        <button className="px-5 text-sm font-medium leading-none border-sky-700 border-solid border rounded text-white hover:text-sky-700 h-10 hover:bg-white bg-sky-700 mx-7"
        href="/">Read More</button>
      </Container>
    </div>
  );
}
